...
- `docs/CAMERA_SESSION_KNOWLEDGE_BASE.md`：改、评审或排查摄像头权限状态机、采集会话生命周期、设备选择、中心舞台或权限降级恢复前**必读**。不读会踩错线程边界或把权限撤销当成普通失败。
- `docs/MIRROR_WINDOW_KNOWLEDGE_BASE.md`：改、评审或排查镜像窗口拖拽/缩放/透明度、位置持久化、圆形裁剪预览或隐藏时机前**必读**。不读会弄坏几何交互或把隐藏时机和采集停启绑错。
- `docs/MENU_BAR_ENTRY_KNOWLEDGE_BASE.md`：改、评审或排查菜单栏入口、状态栏图标、左右键分发、菜单项状态同步、**启动只就绪不采不弹窗**、**禁止隐藏菜单栏图标**、应用生命周期与权限恢复观察前**必读**。不读会在启动时打开摄像头/镜子，或误加「隐藏菜单栏图标」导致唯一入口消失。
- `docs/DISTRIBUTION_KNOWLEDGE_BASE.md`：改、评审、排查或发布首装包、签名、公证、应用内更新前**必读**。不读会发布无法安装或无法升级的版本。

## 2026-09-05 启动只就绪、不采不弹窗

产品裁定（纠正「冷启动也打开镜子」）：

1. **任何自动启动路径（冷启动、登录项）只就绪菜单栏图标与会话对象，禁止 `showMirror()`，禁止打开摄像头。** 摄像头指示灯不得在无人操作时亮起。
2. **仅用户主动操作才开镜子与采集**：菜单栏左键、菜单「显示镜像」。就绪后默认 60 秒内从「应用程序」/ Spotlight 再次打开（reopen）须出**设置窗**（`MenuBarReopenPolicy` + `menubarIsPrimaryEntry`），不自动开摄像头。
3. **禁止隐藏菜单栏图标**：唯一常驻入口是状态栏图标；菜单无「隐藏菜单栏图标」。
4. **右键菜单能力须在设置窗对等**（裁定 2026-09-07）：显示/隐藏镜像、水平镜像、开机自启、检查更新、退出；右键另有「设置…」。

冷启动与登录项对「自动开窗/开摄像头」的禁令相同。MacKit ≥0.1.4。

本机已覆盖安装目标：1.0.12 (13)。冷启动不弹窗/不开摄像头须本机验证。
