...
2. **仅用户主动操作才开镜子与采集**：菜单栏左键、菜单「显示镜像」、以及应用已在跑时从「应用程序」/ Spotlight 再次打开（reopen）。
3. **禁止隐藏菜单栏图标**：唯一常驻入口是状态栏图标；菜单无「隐藏菜单栏图标」。

冷启动与登录项对「自动开窗/开摄像头」的禁令相同，不要再区分「登录静默、手动冷启动却弹窗」。装配路径已不再调用 `LoginLaunchDetector`（禁令统一后无需分支）；`MacKitLifecycle` 仍用于退出守卫等。

本机已覆盖安装目标：1.0.11 (12)。冷启动不弹窗/不开摄像头须本机验证。
