...
- `docs/MENU_BAR_ENTRY_KNOWLEDGE_BASE.md`：改、评审或排查菜单栏入口、状态栏图标、左右键分发、菜单项状态同步、**登录项静默启动**、**禁止隐藏菜单栏图标**、应用生命周期与权限恢复观察前**必读**。不读会把 App Icon 直接缩小成菜单栏图、破坏左键直达镜像、登录时弹出镜子、或误加「隐藏菜单栏图标」导致唯一入口消失。
- `docs/DISTRIBUTION_KNOWLEDGE_BASE.md`：改、评审、排查或发布首装包、签名、公证、应用内更新时**必读**；否则会发布无法安装或无法升级的版本。

## 2026-09-05 登录静默与图标常驻

产品裁定（与全局 macOS 菜单栏指南一致）：

1. **登录项拉起必须静默**：`LoginLaunchDetector.isLaunchedAsLoginItem` 为真时不调用 `showMirror()`；用户主动冷启动仍「打开就看到镜子」。
2. **禁止隐藏菜单栏图标**：唯一常驻入口是状态栏图标；菜单无「隐藏菜单栏图标」，镜像窗无「显示菜单栏图标」开关；启动时若偏好曾为隐藏则写回可见。

本机已覆盖安装 1.0.9 (10)。重新登录真机静默未测。
